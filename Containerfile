...
RUN --mount=type=secret,id=rh_user --mount=type=secret,id=rh_pass \
    ...
    python3-pip lshw pciutils sysstat git ethtool jq linuxptp hwdata synce4l gpsd-minimal gpsd-minimal-clients i2c-tools acpica-tools kernel-tools && dnf clean all && \
