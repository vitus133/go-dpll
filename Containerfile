...
RUN --mount=type=secret,id=rh_user --mount=type=secret,id=rh_pass \
    ...
    gcc make bison flex libmnl-devel libcap-devel elfutils-libelf-devel ncurses-devel libmnl-devel sysfsutils perf rt-tests \
    python3-pip lshw pciutils sysstat git ethtool jq linuxptp hwdata synce4l gpsd-minimal gpsd-minimal-clients i2c-tools && dnf clean all && \
